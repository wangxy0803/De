...
% r in frame {b}
r11 = eval([sol.r11x(1), sol.r11y(1), sol.r11z(1)]');
r12 = eval([sol.r12x(1), sol.r12y(1), sol.r12z(1)]');
r21 = eval([sol.r21x(1), sol.r21y(1), sol.r21z(1)]');
r22 = eval([sol.r22x(1), sol.r22y(1), sol.r22z(1)]');
r31 = eval([sol.r31x(1), sol.r31y(1), sol.r31z(1)]');
r32 = eval([sol.r32x(1), sol.r32y(1), sol.r32z(1)]');


% set r in frame {a}
r11_a = eval(R_cal*[sol.r11x(1), sol.r11y(1), sol.r11z(1)]');
r12_a = eval(R_cal*[sol.r12x(1), sol.r12y(1), sol.r12z(1)]');
r21_a = eval(R_cal*[sol.r21x(1), sol.r21y(1), sol.r21z(1)]');
r22_a = eval(R_cal*[sol.r22x(1), sol.r22y(1), sol.r22z(1)]');
r31_a = eval(R_cal*[sol.r31x(1), sol.r31y(1), sol.r31z(1)]');
r32_a = eval(R_cal*[sol.r32x(1), sol.r32y(1), sol.r32z(1)]');
...
X_old = get_initial_x([-180, 0, 80]', [90, -160, 80]', [90, 160, 80]', leg11, leg12, leg21, leg22, leg31, leg32);
syms theta1 theta2 theta3; 

r12 = [leg12*cos(theta1), 0, -leg12*sin(theta1)]';
r22 = [-1/2*leg22*cos(theta2), sqrt(3)/2*leg22*cos(theta2), -leg22*sin(theta2)]';
r32 = [-1/2*leg32*cos(theta3), -sqrt(3)/2*leg32*cos(theta3), -leg32*sin(theta3)]';

eqn_1 = eval(norm((bbb1 + r11 + r12)-(bbb2 + r21 + r22)))^2 - 3*R_outer_circle*R_outer_circle == 0;
eqn_2 = eval(norm((bbb1 + r11 + r12)-(bbb3 + r31 + r32)))^2 - 3*R_outer_circle*R_outer_circle == 0;
eqn_3 = eval(norm((bbb2 + r21 + r22)-(bbb3 + r31 + r32)))^2 - 3*R_outer_circle*R_outer_circle == 0;

eqns = [eqn_1, eqn_2, eqn_3];
vars = [theta1 theta2 theta3];

% All depend on MATLAB
% use solve function to get analytical solutions
% sol = solve(eqns,vars,"ReturnConditions",true);
sol = solve(eqns,vars);
% use vpasolve function to get numerical solutions
...
[ans_num, ~] = size(sol.theta1);


r12 = eval([leg12*cos(sol.theta1), 0, -leg12*sin(sol.theta1)]');
r22 = eval([-1/2*leg22*cos(sol.theta2), sqrt(3)/2*leg22*cos(sol.theta2), -leg22*sin(sol.theta2)]');
r32 = eval([-1/2*leg32*cos(sol.theta3), -sqrt(3)/2*leg32*cos(sol.theta3), -leg32*sin(sol.theta3)]');
