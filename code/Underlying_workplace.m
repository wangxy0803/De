...
%             py = 350;
            ...
            p = [px, py];
            disp("phi="+num2str(phi)+","+"px="+num2str(px)+","+"py="+num2str(py)+":");
            [d_1x, d_1y_1, d_1y_2, c_1x, c_1y_1, c_1y_2, succeed_final] = Configuration_calcaulate_d_3bar(p, phi/180*pi, b11, a1, 1/2*length, INVALID_VALUE, rectangular_L, leg_1);
            if succeed_final == 0
                continue;
            end
            [d_2x, d_2y_1, d_2y_2, c_2x, c_2y_1, c_2y_2, succeed_final] = Configuration_calcaulate_d_3bar(p, phi/180*pi, b22, a2, 1/2*length, INVALID_VALUE, rectangular_L, leg_2);
            if succeed_final == 0
                continue;
            end
            [d_3x, d_3y_1, d_3y_2, c_3x, c_3y_1, c_3y_2, succeed_final] = Configuration_calcaulate_d_3bar(p, phi/180*pi, b22, a3, 1/2*length, INVALID_VALUE, rectangular_L, leg_3);
            if succeed_final == 0
                continue;
            end
            [d_1y,d_2y,d_3y, succeed_final] = Configuration_select_d(d_1y_1, d_1y_2, d_2y_1, d_2y_2, d_3y_1, d_3y_2, INVALID_VALUE, a2, a3, rectangular_L, 'configuration_1', 'up');
            if succeed_final == 0
                continue;
            end
            [TRUE_OR_FALSE, ~] = check_limitation(distance,R*b22'+p', a1(2),d_1y,leg_1,width);
            if TRUE_OR_FALSE
                phi_list = [phi,phi_list];
                px_list = [px,px_list];
                py_list = [py, py_list];
            else
                continue;
...
axis equal;
num = size(px_list);
c = linspace(1,10,num(2));
scatter(px_list, py_list, 20, c, "filled");
hold on
% draw moving trace
axis equal;
line([-1/2*width, -1/2*width],[-1/4*length, 3/4*length]);
line([1/2*width, 1/2*width],[-1/4*length, 3/4*length]);
line([-1/2*width-rectangular_W, -1/2*width+rectangular_W],[-1/4*length, -1/4*length],'Color','blue','LineWidth',3);
line([-1/2*width-rectangular_W, -1/2*width+rectangular_W],[3/4*length, 3/4*length],'Color','blue','LineWidth',3);
line([1/2*width-rectangular_W, 1/2*width+rectangular_W],[-1/4*length, -1/4*length],'Color','blue','LineWidth',3);
line([1/2*width-rectangular_W, 1/2*width+rectangular_W],[3/4*length, 3/4*length],'Color','blue','LineWidth',3);
